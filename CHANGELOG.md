...
- **自定义路由（`register_route`）此前从未提交** —— 能力只存在于未提交的工作区，
  于是"源码工作树里能跑、`pip install` 出来的包里没有"。症状极具迷惑性：
  服务照报 `healthy`、管理台照常能聊，**只有应用自己注册的接口全部 404，零提示**。
  `pasm-medical` 的全部医疗路由都挂在这上面 —— 也就是说，
  用它发出去的包会"装得上、用不了"。

  **教训**：现有验证全部跑在源码工作树上，从没用过"装出来的包"。
  这类问题只能在**分发形态**（wheel → 干净 venv → 起服务 → 真打接口）上才抓得住。

### 新增

- **`WebGatewayPlugin.register_route` / `match_route`** —— 应用侧扩展接口的运行时
  注册入口。自定义路由与内置路由**共用同一套鉴权与限流**，且**一律管理作用域**
  （能写记忆/数据的接口不能对访客开放）。
- **`WebGatewayPlugin.routes_info()`**，并在 `/healthz` 中暴露 `custom_routes` 条数
  —— "该注册的路由到底有没有注册上"从此一眼可辨（`custom_routes: 0` 就是信号）。
- **`register_route` 拒绝内置路径**：注册到 `/healthz`、`/api/chat`、`/console`、
  `/api/cog/*` 等内置路径会被内置分支遮蔽、**静默失效**；现在启动即报错，
  并提示改用 `/api/app/...` 这类自有前缀。
- selftest 新增 17 项，其中 8 项**真起 HTTP 服务器**验证自定义路由端到端可用
  （命中 / query 与 body 透传 / 未注册 404 / 无令牌 401 / 公开令牌 401 /
  handler ValueError → 400 / 其它异常 → 500 / `/healthz` 计数）。

### 说明

- 网关内对自定义路由存在**两道鉴权**（`do_GET`/`do_POST` 入口一道，`_custom` 内一道），
  **刻意冗余**：入口那道是全部路径共用的，一旦将来有人在它之前插入新分支，
  内层就是兜底。反例对照实测：单独破坏任一道**仍安全**（另一道拦住），
  必须同时破坏才会失败 —— 所以 selftest 断言的是**结果**（公开令牌打自定义路由必须
  401），而不是"某一行代码生效"。别把内层那道当死代码删掉。
